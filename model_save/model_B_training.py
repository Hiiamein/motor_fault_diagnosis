...
train_0_B = np.loadtxt(open("../data/motor_fault/train_0_B.csv","rb"), delimiter=",", skiprows=0)
train_1_B = np.loadtxt(open("../data/motor_fault/train_1_B.csv","rb"), delimiter=",", skiprows=0)
print("train_0_B.shape: ", train_0_B.shape)
print("train_1_B.shape: ", train_1_B.shape)
#print("test_F.shape",test_F.shape)

# 高斯归一化
train_0_B[:,:-1] = preprocessing.scale(train_0_B[:,:-1])
train_1_B[:,:-1] = preprocessing.scale(train_1_B[:,:-1])
...
# Dataset
train_B = np.concatenate((train_0_B, train_1_B), axis=0)
x = torch.tensor(train_B[:,:-1], dtype=torch.float32)
y = torch.tensor(train_B[:,-1], dtype=torch.long)
...
for epoch in range(num_epochs):
    ...
    ## save model
    if epoch > 14:
        torch.save(model, 'checkpoint_B_epoch_{}.pt'.format(epoch+1))
    ...
    print("train_acc: {:.4f}, train_recall: {:.4f}, train_precision: {:.4f}".format(valid_acc, valid_recall, valid_precision))
